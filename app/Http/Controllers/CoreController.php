...
class CoreController extends WebController
{
    ...
    protected function cart_items()
    {
        ...
        $all_items_have_event_menu_category_uid = true;
        ...
        if(auth()->check()) {
            ...
            foreach($order_items as $order_item) {
                ...
                if(!is_null($variation)) {
                    ...
                    // Check if item has event_menu_category_uid
                    if(is_null($order_item->event_menu_category_uid)) {
                        $all_items_have_event_menu_category_uid = false;
                    }
                    
                    // Load event_menu_category if event_menu_category_uid is set
                    $event_menu_category = null;
                    if(!is_null($order_item->event_menu_category_uid)) {
                        $event_menu_category = EventMenuCategory::where('uid', $order_item->event_menu_category_uid)->first();
                        if($event_menu_category) {
                            $order_item['event_menu_category'] = [
                                'uid' => $event_menu_category->uid,
                                'name' => $event_menu_category->name,
                                'pickup_start_date' => $event_menu_category->pickup_start_date,
                                'pickup_end_date' => $event_menu_category->pickup_end_date,
                            ];
                            
                            // Only collect pickup dates if ALL items have event_menu_category_uid
                            if($all_items_have_event_menu_category_uid && !is_null($event_menu_category->pickup_start_date) && !is_null($event_menu_category->pickup_end_date)) {
                                $pickup_start_dates[] = Carbon::parse($event_menu_category->pickup_start_date);
                                $pickup_end_dates[] = Carbon::parse($event_menu_category->pickup_end_date);
                                if(!in_array($variation->product->name, $products_with_date_range)) {
                                    $products_with_date_range[] = $variation->product->name;
                                }
                            }
                        }
                    }
            ...
            foreach($order_items as &$order_item) {
                ...
                if(!is_null($variation)) {
                    ...
                    // Add nested structure for frontend to access variation.product.allow_client_note and event_menu_categories
                    $order_item['variation'] = [
                        'product' => [
                            'allow_client_note' => $variation->product->allow_client_note,
                            'event_menu_categories' => $variation->product->event_menu_categories ? $variation->product->event_menu_categories->map(function($category) {
                                return [
                                    'id' => $category->id,
                                    'pickup_start_date' => $category->pickup_start_date,
                                    'pickup_end_date' => $category->pickup_end_date,
                                ];
                            }) : [],
                        ],
                    ];
                    ...
                    // Check if item has event_menu_category_uid
                    if(empty($order_item['event_menu_category_uid'])) {
                        $all_items_have_event_menu_category_uid = false;
                    }
        ...
        // Only apply restrictions if ALL items have event_menu_category_uid
        $pickup_min_date = null;
        $pickup_max_date = null;
        
        if($all_items_have_event_menu_category_uid && count($pickup_start_dates) > 0) {
            $pickup_min_date = collect($pickup_start_dates)->max();
        }
        if($all_items_have_event_menu_category_uid && count($pickup_end_dates) > 0) {
            $pickup_max_date = collect($pickup_end_dates)->min();
        }
        
        return compact(['total_amount', 'order_items', 'is_weekend_only', 'pickup_min_date', 'pickup_max_date', 'products_with_date_range', 'all_items_have_event_menu_category_uid']);
    }
    ...
    public function cart()
    {
        ...
        // Only apply restrictions if ALL items have event_menu_category_uid
        $all_items_have_event_menu_category_uid = $cart_items['all_items_have_event_menu_category_uid'] ?? false;
        $pickup_min_date = $cart_items['pickup_min_date'] ?? null;
        $pickup_max_date = $cart_items['pickup_max_date'] ?? null;
        
        if($all_items_have_event_menu_category_uid && $pickup_min_date) {
        ...
        if($all_items_have_event_menu_category_uid && $pickup_max_date) {
        ...
        return Inertia::render('Checkout/Cart', compact(['order_items', 'total_amount', 'quick_days', 'first_day_string', 'available_times', 'closed_dates', 'full_name', 'email', 'phone', 'is_auth', 'first_pickup_date', 'last_pickup_date', 'settings', 'order_notes', 'is_weekend_only', 'pickup_date_range', 'products_with_date_range', 'all_items_have_event_menu_category_uid']));
    }
    ...
    public function addToCart(Request $request)
    {
        $data = $request->validate([
            'note' => ['nullable', 'string'],
            'event_menu_category_uid' => ['nullable', 'string'],
        ]);
        ...
        if(!is_null($product) && !is_null($variation)) {
            ...
            $event_menu_category_uid = $data['event_menu_category_uid'] ?? null;
            if(auth()->check()) {
                $user = auth()->user();
                OrderItem::create([
                    'user_id' => $user->id,
                    ...
                    'client_note' => $note,
                    'event_menu_category_uid' => $event_menu_category_uid,
                ]);
