...
<template>
  ...
  <ScreenModal :show="showLandingModal" @close="closeLanding" maxWidth="6xl">

    <template #content>
      <div class="relative">
        <!-- Close button in top right of modal -->
        <v-btn @click.prevent="closeLanding" icon="mdi-close" color="white" variant="text" size="small"
          class="absolute top-4 right-4" style="z-index: 10; position: absolute; top: 0; right: 0;"></v-btn>

        <template v-if="localDetails">
          <v-img :src="`/storage/assets/images/landing/${localDetails.image}`" aspect-ratio="2" color="black" cover
            ...
            <div>
              <v-card style="background: rgba(0,0,0,0.7);" rounded="lg">
                <div class="px-7 py-3">
                  ...
                  <template v-if="false">
                    <v-text-field v-model="newsletterEmail" @keydown.enter="newsletterSubscribe" variant="solo"
                      rounded="0" />
                    ...
                    <v-btn @click.prevent="newsletterSubscribe" :color="getButtonColor">Subscribe</v-btn>
                  </template>
                  <template v-else>

                    <div class="my-2">
                      <v-btn href="https://www.toasttab.com/le-petit-four-bakery-380-washington-street/marketing-signup"
                        :color="getButtonColor" target="_blank">Subscribe</v-btn>
                    </div>
          ...
          <div class="mt-5">
            <v-btn @click.prevent="closeLanding" color="white" size="small" prepend-icon="mdi-close">Dismiss</v-btn>
          </div>
            </Col>
